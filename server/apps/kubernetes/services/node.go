...
type INode interface {
	...
	GetPodByNode(ctx context.Context, nodeName string, query *entity.PageParam) *entity.PageResult
...
func NewNode(cli *cache.ClientConfig, event IEvent, pod IPod) *Node {
	return &Node{cli: cli, Events: event, Pods: pod}
}

func (n *Node) List(ctx context.Context, query *entity.PageParam, name string, label string) *entity.PageResult {
	var nodes = make([]*entity2.Node, 0)
	...
	var nodeList = make([]*entity2.Node, 0)
	if name != "" {
...
func (n *Node) GetPodByNode(ctx context.Context, nodeName string, query *entity.PageParam) *entity.PageResult {
...
func (n *Node) GetNodeEvent(ctx context.Context, name string) []*corev1.Event {
	...
	result := n.Events.List(ctx, &entity.PageParam{
		Page:  0,
		Limit: 0,
	})
