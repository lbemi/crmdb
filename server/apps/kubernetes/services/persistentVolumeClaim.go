...
type PersistentVolumeClaimImp interface {
	...
	ListByStorageClassName(ctx context.Context, query *entity.PageParam, storageClassName string, name string, label string) *entity.PageResult
...
func (p *PersistentVolumeClaim) ListByStorageClassName(ctx context.Context, query *entity.PageParam, storageClassName string, name string, label string) *entity.PageResult {

	pvcs, err := p.client.SharedInformerFactory.Core().V1().PersistentVolumeClaims().Lister().List(labels.Everything())
	restfulx.ErrNotNilDebug(err, restfulx.GetResourceErr)

	data := make([]*v1.PersistentVolumeClaim, 0)
