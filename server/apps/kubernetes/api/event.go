...
func ListEvents(rc *rctx.ReqCtx) {
	...
	rc.ResData = core.V1.Cluster(clusterName).K8S().Events(namespace).List(c, pageParam)
}
...
func GetEvent(rc *rctx.ReqCtx) {
	...
	rc.ResData = core.V1.Cluster(clusterName).K8S().Events(namespace).Get(c, eventName)
}
