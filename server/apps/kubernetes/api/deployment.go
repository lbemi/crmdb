...
func ListDeployments(rc *rctx.ReqCtx) {
	...
	rc.ResData = core.V1.Cluster(clusterName).K8S().Deployments(namespace).List(c, pageParam, name, label)
}
...
func GetDeployment(rc *rctx.ReqCtx) {
	...
	rc.ResData = core.V1.Cluster(clusterName).K8S().Deployments(namespace).Get(c, deploymentName)
}
...
func RollBackDeployment(rc *rctx.ReqCtx) {
	...
	rc.ResData = core.V1.Cluster(clusterName).K8S().Deployments(namespace).RollBack(c, deploymentName, reversion)
}
func ReDeployDeployment(rc *rctx.ReqCtx) {
	...
	namespace := rc.PathParam("namespace")
	deploymentName := rc.PathParam("name")
	deployment := core.V1.Cluster(clusterName).K8S().Deployments(namespace).Get(c, deploymentName)
...
func DeleteDeployment(rc *rctx.ReqCtx) {
	...
	namespace := rc.PathParam("namespace")
	deploymentName := rc.PathParam("name")
	core.V1.Cluster(clusterName).K8S().Deployments(namespace).Delete(c, deploymentName)
}
...
func ScaleDeployments(rc *rctx.ReqCtx) {
	...
	core.V1.Cluster(clusterName).K8S().Deployments(namespace).Scale(c, deploymentName, int32(replica))
}
...
func GetDeploymentPods(rc *rctx.ReqCtx) {
	...
	namespace := rc.PathParam("namespace")
	deploymentName := rc.PathParam("name")
	pods, replicaSets := core.V1.Cluster(clusterName).K8S().Deployments(namespace).GetDeploymentPods(c, deploymentName)
...
func GetDeploymentEvents(rc *rctx.ReqCtx) {
	...
	namespace := rc.PathParam("namespace")
	deploymentName := rc.PathParam("name")
	rc.ResData = core.V1.Cluster(clusterName).K8S().Deployments(namespace).GetDeploymentEvent(c, deploymentName)
}
...
//
//	deploymentList := core.V1.Cluster(clusterName).K8S().Deployments(namespace).Search(c, key, searchType)
//	if err != nil {
