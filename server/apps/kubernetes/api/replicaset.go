...
func ListReplicaSets(rc *rctx.ReqCtx) {
	...
	rc.ResData = core.V1.Cluster(clusterName).K8S().Replicaset(namespace).List(c, pageParam, name, label)
...
func GetReplicaSet(rc *rctx.ReqCtx) {
	...
	rc.ResData = core.V1.Cluster(clusterName).K8S().Replicaset(namespace).Get(c, replicasetName)
