...
type IPolicy interface {
	...
	SetRolePermission(ctx context.Context, roleId uint64, menus *[]entity.Menu) (bool, error)
...
func (c *Policy) SetRolePermission(ctx context.Context, roleId uint64, menus *[]entity.Menu) (bool, error) {
...
func (c *Policy) UpdatePermissions(ctx context.Context, oldPath, oldMethod, newPath, newMethod string) error {
	err := c.db.Model(&config.Rule{}).
		Where("v1 =? and v2 =?", oldPath, oldMethod).
		Updates(&config.Rule{Path: newPath, Method: newMethod}).Error
...
// 设置角色权限
func (c *Policy) setRolePermission(roleId uint64, menus *[]entity.Menu) (bool, error) {
	rules := [][]string{}
