...
func AddMenu(c *gin.Context) {
	...
	// 判断权限是否已存在
	_, err := core.Core.Menu().GetMenuByMenuNameUrl(c, menu.URL, menu.Method)
	if err != gorm.ErrRecordNotFound {
		response.Fail(c, response.ErrCodeFount)
		return
	}

	if _, err := core.Core.Menu().Create(c, &menu); err != nil {
...
func UpdateMenu(c *gin.Context) {
	...
	if !core.Core.Menu().CheckMenusIsExist(c, menuID) {
		response.Fail(c, response.ErrCodeNotFount)
		return
	}

	if err := core.Core.Menu().Update(c, &menu, menuID); err != nil {
		response.Fail(c, response.ErrOperateFailed)
		return
	}
...
func DeleteMenu(c *gin.Context) {
	...
	if err := core.Core.Menu().Delete(c, menuID); err != nil {
		response.Fail(c, response.ErrOperateFailed)
		return
	}
...
func GetMenu(c *gin.Context) {
	...
	res, err := core.Core.Menu().Get(c, menuID)
	if err != nil {
		response.Fail(c, response.ErrOperateFailed)
		return
	}
...
func ListMenus(c *gin.Context) {
	...
	res, err := core.Core.Menu().List(c, page, limit, menuType)
	if err != nil {
		response.Fail(c, response.ErrOperateFailed)
		return
	}
...
func UpdateMenuStatus(c *gin.Context) {
	...
	if err := core.Core.Menu().UpdateStatus(c, menuID, status); err != nil {
		response.Fail(c, response.ErrCodeParameter)
		return
	}
