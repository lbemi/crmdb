...
func ClusterRoutes() *restful.WebService {
	...
	ws.Route(ws.GET("").To(func(request *restful.Request, response *restful.Response) {
		rctx.NewReqCtx(request, response).WithLog("cluster").WithHandle(v1beat1.ListCluster).Do()
	...
	ws.Route(ws.GET("/{name}").To(func(request *restful.Request, response *restful.Response) {
		rctx.NewReqCtx(request, response).WithLog("cluster").WithHandle(v1beat1.GetCluster).Do()
	...
	ws.Route(ws.POST("").To(func(request *restful.Request, response *restful.Response) {
		rctx.NewReqCtx(request, response).WithLog("cluster").WithHandle(v1beat1.CreateCluster).Do()
	...
	ws.Route(ws.DELETE("/{id}").To(func(request *restful.Request, response *restful.Response) {
		rctx.NewReqCtx(request, response).WithLog("cluster").WithHandle(v1beat1.DeleteCluster).Do()
