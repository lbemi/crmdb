				...
				<el-table-column fixed="right" label="操作" width="260px" flex>
					<template #default="scope">
						...
						<el-button
							...
							type="danger"
							:size="state.size"
							@click="deletePipelinerun(scope.row)"
							>删除</el-button
						>
					</template>
				</el-table-column>
...
<script setup lang="ts" name="pipelinerun">
...
import { useTektonPipelineRunsApi } from '@/api/tekton/pipelineRuns';

import { Pipelinerun } from '@/types/tekton/pipelinerun';

const Pagination = defineAsyncComponent(() => import('@/components/pagination/pagination.vue'));
const YamlDialog = defineAsyncComponent(() => import('@/components/yaml/index.vue'));

const pipelinerunApi = useTektonPipelineRunsApi();
...
const deletePipelinerun = (pipelinerun: Pipelinerun) => {
	state.loading = true;
	ElMessageBox({
		title: '提示',
		message: h('p', null, [
			h('span', null, '此操作将删除 '),
			h('i', { style: 'color: teal' }, `${pipelinerun.metadata!.name}`),
			h('span', null, ' 任务. 是否继续? '),
		]),
		buttonSize: 'small',
		...
		.then(() => {
			pipelinerunApi
				.deletePipelinerun(pipelinerun.metadata.namespace, pipelinerun.metadata.name, {
					cloud: k8sStore.state.activeCluster,
				})
				.then((res: any) => {
					listPipelineruns();
...
const listPipelineruns = () => {
	state.loading = true;
	pipelinerunApi
		.listPipelinerun(k8sStore.state.activeNamespace, state.query)
		.then((res: any) => {
			state.pipelineruns = res.data.data;
