		...
		<el-card shadow="hover" class="layout-padding-auto card">
			<el-row :gutter="20">
				<el-col :span="18">
					...
					<span style="font-weight: 35">{{ k8sStore.state.activeJob?.metadata?.name }}</span></el-col
				>
				<el-col :span="6"
					><el-button v-auth="'k8s:job:edit'" type="primary" size="small" :icon="Edit" @click="handleEdit()">编辑</el-button>
					<el-button type="primary" size="small" :icon="View" @click="showYaml">查看YAML</el-button>
					<el-button v-auth="'k8s:job:redeploy'" type="primary" size="small" :icon="Refresh" @click="reDeploy">重新部署</el-button>
			...
			<el-descriptions :column="3" border class="desc-body">
				<el-descriptions-item label="名称" label-align="right" align="center" label-class-name="my-label" class-name="my-content" width="150px">{{
					k8sStore.state.activeJob?.metadata?.name
				}}</el-descriptions-item>
				<el-descriptions-item label="命名空间" label-align="right" align="center">{{
					k8sStore.state.activeJob?.metadata?.namespace
				}}</el-descriptions-item>
				<el-descriptions-item label="创建时间" label-align="right" align="center">{{
					dateStrFormat(k8sStore.state.activeJob?.metadata?.creationTimestamp?.toString() || '')
				}}</el-descriptions-item>
				<el-descriptions-item label="并行pod数" label-align="right" align="center"
					><el-button
						v-auth="'k8s:job:scale'"
						color="#626aef"
						:icon="Minus"
						size="small"
						plain
						style="margin-right: 10px"
						@click="scaleDeploy('minus')" />{{ k8sStore.state.activeJob?.spec?.parallelism
					}}<el-button v-auth="'k8s:job:scale'" color="#626aef" :icon="Plus" size="small" plain @click="scaleDeploy('plus')"
				/></el-descriptions-item>
				<el-descriptions-item label="镜像" label-align="right" align="center">
					<div class="tag-center">
						<el-tag round effect="plain" v-for="(item, index) in k8sStore.state.activeJob?.spec?.template?.spec?.containers" :key="index">{{
							item.image?.split('@')[0]
						}}</el-tag>
					</div>
				</el-descriptions-item>
				<el-descriptions-item label="滚动升级策略" label-align="right" align="center">
					<div>
						期望完成次数：
						{{ k8sStore.state.activeJob?.spec?.completions }}
					</div>
					<div>
						最大失败次数：
						{{ k8sStore.state.activeJob?.spec?.backoffLimit }}
					</div>
				</el-descriptions-item>
				<el-descriptions-item label="策略" label-align="right" align="center">{{
					k8sStore.state.activeJob?.spec?.completionMode
				}}</el-descriptions-item>
				<el-descriptions-item label="状态" label-align="right" align="center">
					成功：<a v-if="k8sStore.state.activeJob?.status?.succeeded">{{ k8sStore.state.activeJob?.status?.succeeded }}</a> 个，活动：{{
						k8sStore.state.activeJob?.status?.active || '0'
					}}
					个，失败：
					<a style="color: red" v-if="k8sStore.state.activeJob?.status?.failed">{{ k8sStore.state.activeJob?.status?.failed }}</a>
					个
			...
			<div v-show="data.iShow">
				<el-divider />
				<el-table :data="k8sStore.state.activeJob?.status?.conditions" stripe style="width: 100%">
					<el-table-column prop="type" label="类型" />
					<el-table-column prop="status" label="状态" />
					<el-table-column label="开始时间">
						<template #default="scope">
							{{ dateStrFormat(scope.row.lastProbeTime) }}
						</template>
					</el-table-column>
			...
			<el-tabs v-model="data.activeName" class="demo-tabs" @tab-click="handleClick">
				<el-tab-pane label="容器组" name="first">
					<el-table :data="data.pods" stripe style="width: 100%" max-height="350px">
						<el-table-column prop="metadata.name" label="名称" show-overflow-tooltip>
							<template #default="scope">
				...
				<el-tab-pane label="元数据" name="second">
					<MetaDetail :metaData="k8sStore.state.activeJob.metadata" />
				</el-tab-pane>
				<el-tab-pane label="环境变量" name="third">
					<el-descriptions :column="1" direction="vertical">
						<el-descriptions-item
							:label="'容器: ' + item.name"
							v-for="(item, index) in k8sStore.state.activeJob.spec?.template.spec?.containers"
							:key="index"
						>
...
<script lang="ts" setup name="k8sJobDetail">
...
const { refreshActiveJob } = kubernetesInfo();
...
//编辑job
const handleEdit = () => {
	k8sStore.state.creatJob.namespace = k8sStore.state.activeJob.metadata!.namespace!;
	const dep = deepClone(k8sStore.state.activeJob) as Job;
	delete dep.status;
	delete dep.metadata?.managedFields;
	data.job = dep;
	data.update.title = '更新job';
...
const rollBack = (rs: ReplicaSet) => {
	ElMessageBox({
		...
		message: h('p', null, [
			h('span', null, '回滚到 '),
			h('i', { style: 'color: teal' }, `${rs.metadata?.annotations!['job.kubernetes.io/revision']}`),
			h('span', null, ' 版本. 是否继续? '),
		]),
		buttonSize: 'small',
		...
		.then(() => {
			jobApi
				.rollBackJob(
					k8sStore.state.activeJob.metadata!.namespace!,
					k8sStore.state.activeJob.metadata!.name!,
					rs.metadata!.annotations!['job.kubernetes.io/revision'],
					{
						cloud: k8sStore.state.activeCluster,
					}
				)
...
const reDeploy = () => {
	const job = k8sStore.state.activeJob;
	ElMessageBox({
		title: '提示',
		message: h('p', null, [
			h('span', null, '重新部署 '),
			h('i', { style: 'color: teal' }, `${job.metadata?.name}`),
			h('span', null, ' . 是否继续? '),
		]),
		buttonSize: 'small',
		showCancelButton: true,
...
const scaleDeploy = (action: string) => {
	if (action === 'plus') {
		k8sStore.state.activeJob.spec!.replicas!++;
	} else {
		k8sStore.state.activeJob.spec!.replicas!--;
	}
	jobApi
		.scaleJob(k8sStore.state.activeJob.metadata!.namespace!, k8sStore.state.activeJob.metadata!.name!, k8sStore.state.activeJob.spec?.replicas!, {
			cloud: k8sStore.state.activeCluster,
		})
		.then(() => {
...
const getPods = async () => {
	const res = await jobApi.detailJob(
		k8sStore.state.activeJob.metadata!.namespace!.toString(),
		k8sStore.state.activeJob?.metadata!.name!.toString(),
		data.param
	);
...
const getEvents = async () => {
	const res = await jobApi.getJobEvents(
		k8sStore.state.activeJob.metadata!.namespace!.toString(),
		k8sStore.state.activeJob?.metadata!.name!.toString(),
		data.param
	);
...
const showYaml = async () => {
	delete k8sStore.state.activeJob.metadata?.managedFields;
	data.codeData = k8sStore.state.activeJob;
	data.dialogVisible = true;
};
const buildWebsocket = () => {
	const ws = websocketApi.createWebsocket('job');
	...
	ws.onmessage = (e) => {
		if (e.data === 'ping') {
			...
			if (
				object.type === 'job' &&
				object.result.namespace === k8sStore.state.activeJob?.metadata?.namespace &&
				object.cluster == k8sStore.state.activeCluster
			) {
				data.jobs = object.result.data;
				data.jobs.forEach((item: Job) => {
					if (item.metadata!.name == k8sStore.state.activeJob?.metadata?.name) {
						k8sStore.state.activeJob = item;
						return;
					}
				});
			}
