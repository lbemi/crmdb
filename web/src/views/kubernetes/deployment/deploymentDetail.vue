<template>
    <el-descriptions :title="depStore.activeDeployment?.metadata.name" :column="3" border>
        <el-descriptions-item label="名称" label-align="right" align="center" label-class-name="my-label"
            class-name="my-content" width="150px">{{ depStore.activeDeployment?.metadata.name }}</el-descriptions-item>
        <el-descriptions-item label="命名空间" label-align="right" align="center">{{
            depStore.activeDeployment?.metadata.namespace
        }}</el-descriptions-item>
        <el-descriptions-item label="副本数" label-align="right" align="center">{{ depStore.activeDeployment?.spec.replicas
        }}</el-descriptions-item>
        <el-descriptions-item label="创建时间" label-align="right" align="center">{{
            $filters.dateFormat(depStore.activeDeployment?.metadata.creationTimestamp) }}</el-descriptions-item>
        <el-descriptions-item label="选择器" label-align="right" align="center">
            <div class="tag-center">
                <el-tag effect="plain" round
                    v-for="(item, key, index) in depStore.activeDeployment?.spec.selector.matchLabels" :key="index">
                    {{ key }}:{{ item }}
                </el-tag>
            </div>
        </el-descriptions-item>
        <el-descriptions-item label="镜像" label-align="right" align="center">
            <div class="tag-center">
                <el-tag round effect="plain"
                    v-for="(item, index) in depStore.activeDeployment?.spec.template.spec.containers" :key="index">{{
        ...
        <el-descriptions-item label="注解" label-align="right" align="center">
            <div class="tag-center"><el-tag effect="plain" type="info"
                    v-for="(item, key, index) in depStore.activeDeployment?.metadata.annotations" :key="index"> {{ key
                    }}:{{
    item }}</el-tag></div>

        </el-descriptions-item>
        <el-descriptions-item label="滚动升级策略" label-align="right" align="center">
            <div>超过期望的Pod数量： {{ depStore.activeDeployment?.spec.strategy.rollingUpdate.maxSurge }}</div>
            <div>不可用Pod最大数量： {{ depStore.activeDeployment?.spec.strategy.rollingUpdate.maxUnavailable }}</div>
        ...
        <el-descriptions-item label="状态" label-align="right" align="center">
            就绪：<a v-if="depStore.activeDeployment?.status.readyReplicas">{{ depStore.activeDeployment?.status.readyReplicas
            }}</a>
            <a style="color: red;" v-else>0</a> /{{ depStore.activeDeployment?.status.replicas }}
            个，已更新：{{ depStore.activeDeployment?.status.updatedReplicas }} 个，可用：
            <a v-if="depStore.activeDeployment?.status.readyReplicas">{{ depStore.activeDeployment?.status.readyReplicas
            }}</a> <a style="color: red;" v-else>0</a>

            个
            <el-link type="primary" :underline="false" @click="iShow = !iShow"
                style="font-size: 10px; margin-left: 5px;">展开现状现状详情<el-icon>
                    <CaretBottom />
                </el-icon></el-link>
        </el-descriptions-item>
    </el-descriptions>
    ...
    <div iShow="iShow">
        <el-divider />
        <el-table :data="depStore.activeDeployment?.status.conditions" stripe style="width: 100%">
        ...
        <el-tab-pane label="容器组" name="first">
            <el-table :data="pods" stripe style="width: 100%">
                ...
                <el-table-column fixed="right" label="操作" width="160">
                    <template #default="scope">
                        <el-button link type="primary" size="small" @click="handleClick">详情</el-button><el-divider
                            direction="vertical" />
    ...
    ws.onmessage = (e) => {
        if (e.data === 'ping') {
            ...
            if (
                ...
                object.result.namespace === depStore.activeDeployment?.metadata.namespace &&
