<template>
  ...
  <el-select
    v-model="selectContiner"
    class="m-2"
    placeholder="选择容器"
    size="large"
  >
    <el-option
      v-for="item in pod?.spec.containers"
      :key="item.name"
      :label="item.name"
      :value="item.name"
    />
...
const getLog = async (
  ...
  console.log('--------------')
  // http://127.0.0.1:8080/api/v1/pod/log/default/n-6b44d89665-gfttb/nginxx?cloud=3.11http://127.0.0.1:8080/api/v1/pod/log/default/n-6b44d89665-gfttb/nginxx?cloud=3.11
  service.request({
    url:
      '/pod/log/' +
      namespace +
